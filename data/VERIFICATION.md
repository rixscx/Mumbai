...
**Status: in progress. 26 entries, 12 of 21 clusters.** The target is ~180 across ≥18 clusters. This
...
4. **Unverified fields ship empty.** Ten of twenty-six entries have no price and three have no
...
| `mahim-bandra-west` | 1 | 0 | 1 | 0 |
| `sewri-mazgaon` | 1 | 0 | 1 | 0 |
| **9 other clusters** | **0** | — | — | — |
| **Total** | **26** | **5** | **20** | **1** |

At zero: `bandra-east-bkc`, `khar-santacruz`, `andheri-versova-jvlr`, `juhu`, `powai-vikhroli`,
`sion-wadala`, `worli-prabhadevi-lower-parel`, `navi-mumbai`, `thane`.

Field completeness across the 26: **geo unresolved 26/26** (blocked on network, resolver written),
hours unknown 3/26, cost unknown 10/26.
...
**The most consequential gap is `worli-prabhadevi-lower-parel` at zero**, because the Nehru Science
Centre and Nehru Planetarium sit there and they are the obvious missing answer to a washed-out
afternoon. `mahim-bandra-west` at one entry is the next, and it degrades Day 6 of the itinerary
directly.
...
Seven things have surfaced across both passes that a generic dataset would have missed, and each one
...
3. **The Monday problem is bigger than one restaurant.** `ideal-corner-fort` and
   `kanheri-caves-sgnp` are *also* shut Mondays. Both trip bookend days are Mondays, so three of the
   best entries in the dataset are unavailable on two of the nine days. The Monday-closure list is
   certainly still incomplete.
...
- **Festival dates for August 2026 — the highest-value open question in the project.** Ganesh
  Chaturthi, Parsi New Year (Pateti) and Janmashtami dates are all unknown to me and have not been
  guessed. **If Ganesh Chaturthi falls inside 10–16 August it changes the entire trip** — Lalbaug,
  the Girgaon workshops, immersion processions, city-wide crowding and transport — and
  `docs/ITINERARY.md` would need rebuilding around it. Parsi New Year would affect five Irani and
  Parsi entries already in the dataset.
- **Tide times for 10–17 August 2026.** The Haji Ali causeway submerges at high tide, Marine Drive's
  overtopping is a high-tide phenomenon, and Sewri's mudflats are tide-dependent. Real predictions
  have to be sourced. No plausible-looking tide table will be written.
- **Independence Day, Saturday 15 August.** Restrictions around CSMT, the Gateway, Marine Drive and
  ...
  `csmvs-kala-ghoda`.
- **Monday closures beyond the three now known.** Both bookend days are Mondays and the list is
